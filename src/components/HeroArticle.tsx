...
export default function HeroArticle({ article }: { article: Article }) {
  return (
    <section className="border-b border-platinum bg-navy">
      <div className="mx-auto grid max-w-6xl gap-0 px-4 py-8 sm:px-6 lg:grid-cols-2 lg:gap-10 lg:px-8 lg:py-12">
        <Link
          href={`/article/${article.slug}`}
          className="relative block aspect-video w-full overflow-hidden bg-platinum lg:aspect-auto"
        >
          <Image
        ...
        <div className="flex flex-col justify-center gap-4 py-6 lg:py-0">
          ...
          <h1 className="text-3xl font-bold leading-tight text-white sm:text-4xl">
            <Link href={`/article/${article.slug}`} className="hover:text-platinum">
              {article.title}
            </Link>
          </h1>
          <p className="text-base leading-relaxed text-platinum">{article.excerpt}</p>
          <Link
            href={`/article/${article.slug}`}
